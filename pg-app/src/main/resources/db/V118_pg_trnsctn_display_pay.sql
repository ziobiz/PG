...
COMMENT ON COLUMN pg_trnsctn.display_cur_type IS '고객통화(ISO). PG 청구 통화(cur_type)와 다를 수 있음(URL DISPLAY_FX·노티 DisplayCurrency 등)';
COMMENT ON COLUMN pg_trnsctn.display_amt IS '고객금액(주단위·소수). PG 청구 금액(amt_krw)과 통화가 다를 때';
