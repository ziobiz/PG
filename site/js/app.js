...
(function () {
  ...
  function pgRefreshCalcCycleSelects(rootEl, preferredValue, parentOrgUnitId) {
    if (!window.PG_API || !window.PG_API.hqSettlementCycleOptions) return Promise.resolve();
    var scopeKey = (parentOrgUnitId != null && String(parentOrgUnitId).trim() !== '')
      ? String(parentOrgUnitId).trim() : '';
      ...
      roots.forEach(function (root) {
        root.querySelectorAll('select[name="calcCycle"]').forEach(function (sel) {
          ...
          var html = opts.map(function (o) {
            ...
            var ttl = o.d ? ' title="' + pgEscapeAttr(String(o.d)) + '"' : '';
    ...
    if (scopeKey && window.PG_API.hqSettlementCycleOptionsScoped) {
      ...
      if (window._pgCalcCycleOptsScopedCache[scopeKey]) {
        applyOptsFromPack(window._pgCalcCycleOptsScopedCache[scopeKey]);
        return Promise.resolve();
      }
      return window.PG_API.hqSettlementCycleOptionsScoped({ parentOrgUnitId: scopeKey }).then(function (pack) {
        window._pgCalcCycleOptsScopedCache[scopeKey] = pack;
        ...
        if (t && t.id === 'hqNmSelPg') {
          ...
          rootEl._hqNmPgInboundTimer = setTimeout(function () {
            ...
            window.PG_API.hqNotifyMappingInboundKeys(pgi, 200).then(function (d) {
              ...
              pane._hqNmGuiScratch = { pg: pgi, ch: ch0, rows: fms0 };
              syncStateToTextareas();
              render();
            });
          }, 400);
  ...
  function bindScreenEvents(pane, tabId) {
    ...
    function doSearch(p, tid, pageOverride) {
      ...
      promise.then(function (data) {
        ...
        if (url === '/commission/commisionList') {
          ...
          /** 체크·번호·업체·거래일만 고정. 거래시간·루트·승인번호·거래번호(우리) 등은 VIEW SETTING과 동일하게 토글 */
          fixedKeys = ['_chk', 'rowNo', 'compNm', 'compId', 'trnDate'];
        }
            ...
            cols.forEach(function (c) {
              if (c.type === 'checkbox') html += '<td><input type="checkbox" class="grid-row-check"></td>';
                ...
                if (url === '/calc/calcGmList' || url === '/settlement/franchiseList'
                  ...
                  var feeCls = [];
                  if (['amount', 'txnFixedFeesSum', 'pctFeesSum', 'usdtFee', 'fxFee', 'fee3dsFee', 'failFee', 'cancelFee', 'voidFee', 'manualVoidFee', 'refundFee', 'chargebackFee', 'rollingPctPlain', 'rollingDays', 'rollingHoldEst', 'totalFee', 'feeVat', 'expectedPayout'].indexOf(c.key) >= 0) feeCls.push('text-end');
                  if (['payCur', 'policyCur', 'vatAppliedYn'].indexOf(c.key) >= 0) feeCls.push('text-center');
                ...
                if (isCompMngTree && c.key === 'rowNo') {
                  ...
                  if (c.key === 'usdtFee' || c.key === 'fxFee' || c.key === 'fee3dsFee') {
                    ...
                    feeShow = feeSpecNum !== 0 ? fmtNum(row[c.key]) : '—';
                  } else if (['amount', 'txnFixedFeesSum', 'pctFeesSum', 'failFee', 'cancelFee', 'voidFee', 'manualVoidFee', 'refundFee', 'chargebackFee', 'rollingHoldEst', 'totalFee', 'feeVat', 'expectedPayout'].indexOf(c.key) >= 0) {
        ...
        if (url === '/calc/feeList' || url === '/settlement/feeList') {
          var feeSum = { totalFee: 0, vat: 0, expected: 0 };
          list.forEach(function (r) {
    ...
    if (viewHelloBtn && pane.querySelector('#tableColumnGuide') && !viewHelloBtn._viewHelloBound) {
      viewHelloBtn._viewHelloBound = true;
      pane._viewSettingHelloHidden = false;
      function syncViewHelloAppearance() {
        ...
        viewHelloBtn.title = hidden ? '클릭 시 VIEW SETTING 패널을 다시 표시합니다.' : '클릭 시 VIEW SETTING 패널을 숨깁니다(저장된 열 구성으로 목록만 표시).';
      }
      syncViewHelloAppearance();
      viewHelloBtn.addEventListener('click', function () {
        var g = pane.querySelector('#tableColumnGuide');
        if (!g) return;
        if (!pane._viewSettingHelloHidden) {
          pane._viewSettingHelloHidden = true;
    ...
    if (url === '/comp/compReg') {
      ...
      if (compDivEl && !compDivEl._merchantToggleBound) {
        ...
        compDivEl.addEventListener('change', function () {
          ...
          if (rf) {
            var pe = rf.querySelector('input[name="parentId"]');
            var pid = (this.value === 'MERCHANT' && pe && pe.value) ? pe.value : null;
        ...
        (function () {
          if (!form) return;
          var pe = form.querySelector('input[name="parentId"]');
          var pid = (compDivEl.value === 'MERCHANT' && pe && pe.value) ? pe.value : null;
      ...
      parentCompSearchBtn.addEventListener('click', function () {
        ...
        function selectParentComp(tr) {
          ...
          ['#compRegForm', '#compDetailForm'].forEach(function (fid) {
            ...
            if (div === 'MERCHANT') {
              var pid = (f.querySelector('input[name="parentId"]') || {}).value;
              pgRefreshCalcCycleSelects(f, undefined, pid || null);
            }
          });
        }
    ...
    function loadCompDetailIntoForm(pane, compId) {
      ...
      window.PG_API.compDetail(compId).then(function (data) {
        ...
        (function () {
          var pcid = (data.compDiv === 'MERCHANT' && data.parentId != null && String(data.parentId) !== '')
            ? String(data.parentId) : null;
          pgRefreshCalcCycleSelects(form, data.calcCycle, pcid).catch(function () {});
        })();
    ...
    if (url === '/hq/notifyMapping' && !pane._hqNotifyMappingBound) {
      ...
      window.PG_API.hqNotifyMapping().then(function (d) {
        ...
        alert(e && e.message ? e.message : '노티 매핑 설정을 불러오지 못했습니다. API 연결·로그인·서버 배포를 확인하세요. 편집 화면은 빈 상태로 열립니다.');
      }).finally(function () {
    ...
    if (url === '/hq/ledgerSysSettings') {
      ...
      function renderHqDataRetentionRows(rows) {
        ...
        rows.forEach(function (r) {
          ...
          bSave.type = 'button';
          bSave.className = 'btn btn-outline-primary hq-dr-row-save';
          bSave.setAttribute('data-id', r.id || '');
          bSave.textContent = '저장';
          var bRev = document.createElement('button');
          bRev.type = 'button';
          bRev.className = 'btn btn-outline-secondary hq-dr-row-revert';
          bRev.setAttribute('data-id', r.id || '');
          bRev.textContent = '수정';
          var bRst = document.createElement('button');
          bRst.type = 'button';
          bRst.className = 'btn btn-outline-danger hq-dr-row-reset';
