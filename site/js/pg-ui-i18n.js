...
(function (g) {
  ...
  var STATIC = {
    ...
    '「활성(직접 입력)」일 때만 직접 입력 가능합니다. 「기본」은 3DS 안전 결제 문구가 언어별로 표시됩니다. 로고설정과 무관하게 단독 표시할 수 있습니다.': {
      EN: 'Custom text is editable only when Active. Default shows 3DS secure payment per language. Can be shown independently of logo setting.',
      JP: '「有効(直接入力)」時のみ入力可。「基本」は言語別の3DSセキュア決済を表示。ロゴ設定とは無関係に単独表示できます。',
      CH: '仅「启用(自定义)」时可编辑。「默认」按语言显示 3DS 安全支付。可与 Logo 设置无关单独显示。',
      TH: 'แก้ไขได้เมื่อเลือก「เปิดใช้งาน(กำหนดเอง)」 「ค่าเริ่มต้น」แสดง 3DS ตามภาษา แสดงแยกจากการตั้งค่าโลโก้ได้'
    },
    ...
    '환불 | 취소 | 무효': {
      EN: 'Refund | Cancel | Void',
      JP: '返金 | 取消 | 無効',
      CH: '退款 | 取消 | 无效',
      TH: 'คืนเงิน | ยกเลิก | เป็นโมฆะ'
    },
    '성공': {
      EN: 'Success',
      JP: '成功',
    ...
    '통화구분': { EN: 'Currency', JP: '通貨区分', CH: '货币类型', TH: 'สกุลเงิน' },
    'searchOrgUnitLabel': { EN: 'Organization', JP: '組織', CH: '组织', TH: 'องค์กร' },
    '가맹 헤더 하단에는 「업체코드 / 소속 총판명」이 표시됩니다.': {
      EN: 'Merchant column headers show merchant code / master distributor name on the second line.',
      JP: '加盟店列ヘッダー2行目に「店舗コード / 所属総販名」を表示します。',
      CH: '商户列标题第二行显示「商户代码 / 所属总代名称」。',
      TH: 'หัวคอลัมน์ร้านค้าแสดง 「รหัสร้าน / ชื่อตัวแทนหลักที่สังกัด」 ในบรรทัดที่สอง'
    },
    ...
    '한번에 최대 위로': {
      EN: 'Move selected to top',
      JP: '選択を最上部へ',
      CH: '所选一次性置顶',
      TH: 'ย้ายที่เลือกไปบนสุด'
    },
    '한번에 가장 아래로': {
      EN: 'Move selected to bottom',
      JP: '選択を最下部へ',
      CH: '所选一次性置底',
      TH: 'ย้ายที่เลือกไปล่างสุด'
    },
    순위변경: {
      EN: 'Rank change',
      JP: '順位変更',
    ...
    '정리할 행을 왼쪽 선택(☑)에서 체크하세요.': {
      EN: 'Check rows to organize using the left selection (☑) column.',
      JP: '整理する行を左の☑で選択してください。',
      CH: '请在左侧选择(☑)列勾选要整理的行。',
      TH: 'ติ๊กแถวที่จะจัดเรียงในคอลัมน์เลือก(☑)ด้านซ้าย'
    },
    '복원할 이전 상태가 없습니다.': {
      EN: 'No previous state to restore.',
      JP: '復元する以前の状態がありません。',
      CH: '没有可恢复的先前状态。',
      TH: 'ไม่มีสถานะก่อนหน้าให้คืนค่า'
    },
    '정책 목록 API를 사용할 수 없습니다. 페이지를 새로고침하세요.': {
      EN: 'Policy list API is unavailable. Please refresh the page.',
      JP: 'ポリシー一覧 API を利用できません。ページを再読み込みしてください。',
      CH: '无法使用策略列表 API。请刷新页面。',
      TH: 'ไม่สามารถใช้ API รายการนโยบายได้ โปรดรีเฟรชหน้า'
    },
    '일괄 선택': {
      EN: 'Select all',
      JP: '一括選択',
    ...
    총본사따름: {
      EN: 'Follow root HQ default',
      JP: '総本部に従う',
      CH: '跟随总本部',
      TH: 'ตามสำนักใหญ่'
    },
    본사따름: {
      EN: 'Follow regional HQ',
      JP: '本社に従う',
      CH: '跟随本社',
      TH: 'ตามสำนักงานใหญ่'
    },
    총판따름: {
      EN: 'Follow master distributor',
      JP: '総代理に従う',
      CH: '跟随总代',
      TH: 'ตามตัวแทนหลัก'
    },
    '배포된 항목만 VIEW SETTING에 노출됩니다. 선택된 항목은 최초 접속 시 기본 ON입니다. 헬로(Hello)로 사용자가 저장한 항목은 조직 설정보다 우선합니다. ▲▼·일괄 이동은 왼쪽 선택(☑) 행에 적용됩니다.': {
      EN: 'Only deployed items appear in VIEW SETTING. Selected items are ON by default on first visit. User saves via Hello override org settings. ▲▼ and bulk move apply to rows checked on the left.',
      JP: '配布項目のみ VIEW SETTING に表示されます。選択項目は初回アクセス時に既定 ON です。Hello でユーザーが保存した項目は組織設定より優先されます。▲▼・一括移動は左の☑行に適用されます。',
      CH: '仅已部署项出现在 VIEW SETTING。已选择项在首次访问时默认开启。用户通过 Hello 保存的项优先于组织设置。▲▼ 与批量移动作用于左侧☑行。',
      TH: 'เฉพาะรายการที่ deploy จะแสดงใน VIEW SETTING รายการที่เลือกจะ ON ตั้งแต่เข้าครั้งแรก การบันทึกผ่าน Hello ของผู้ใช้มีความสำคัญกว่าการตั้งค่าองค์กร ▲▼ และย้ายกลุ่มใช้กับแถว☑ซ้าย'
    },
    '배포된 항목만 VIEW SETTING에 노출됩니다. 선택된 항목은 최초 접속 시 기본 ON입니다. 헬로(Hello)로 사용자가 저장한 항목은 조직 설정보다 우선합니다. 순위변경(▲▼)은 한 칸 이동, 빠른변경은 해당 행을 맨 위·맨 아래로 이동합니다. 정리는 왼쪽 선택(☑) 행을 상단으로 모으며, 복원은 불러오기·저장 직후 상태로 되돌립니다.': {
      EN: 'Only deployed items appear in VIEW SETTING. Selected items are ON by default on first visit. User saves via Hello override org settings. Rank change (▲▼) moves one step; Quick change moves that row to the top or bottom. Organize gathers left ☑ rows to the top; Restore reverts to the state right after load or save.',
      JP: '配布項目のみ VIEW SETTING に表示されます。選択項目は初回アクセス時に既定 ON です。Hello でユーザーが保存した項目は組織設定より優先されます。順位変更(▲▼)は1段移動、クイック変更は当該行を最上・最下へ移動します。整理は左☑行を上部に集め、復元は読込・保存直後の状態に戻します。',
      CH: '仅已部署项出现在 VIEW SETTING。已选择项在首次访问时默认开启。用户通过 Hello 保存的项优先于组织设置。顺序调整(▲▼)移动一格；快速调整将该行移到最上或最下。整理将左侧☑行集中到顶部；恢复回到加载或保存后的状态。',
      TH: 'เฉพาะรายการที่ deploy จะแสดงใน VIEW SETTING รายการที่เลือกจะ ON ตั้งแต่เข้าครั้งแรก การบันทึกผ่าน Hello ของผู้ใช้มีความสำคัญกว่าการตั้งค่าองค์กร เปลี่ยนลำดับ(▲▼)เลื่อนหนึ่งช่อง เปลี่ยนเร็วย้ายแถวนั้นไปบนสุดหรือล่างสุด จัดเรียงรวมแถว☑ซ้ายไว้บน คืนค่ากลับสู่สถานะหลังโหลดหรือบันทึก'
    },
    '배포 열 수': {
      EN: 'Deployed columns',
      JP: '配布列数',
      CH: '部署列数',
      TH: 'จำนวนคอลัมน์ที่ deploy'
    },
    '기본 선택 열 수': {
      EN: 'Default selected columns',
      JP: '既定選択列数',
      CH: '默认选择列数',
      TH: 'จำนวนคอลัมน์ที่เลือกเริ่มต้น'
    },
    '배포된 항목이 하나도 없습니다. 저장할까요?': {
      EN: 'No deployed items. Save anyway?',
      JP: '配布項目がありません。保存しますか？',
      CH: '没有已部署项。仍要保存吗？',
      TH: 'ไม่มีรายการที่ deploy บันทึกต่อไหม?'
    },
    '이동할 행을 왼쪽 선택(☑)에서 체크하세요.': {
      EN: 'Check rows to move using the left selection (☑) column.',
      JP: '移動する行を左の☑で選択してください。',
      CH: '请在左侧选择(☑)列勾选要移动的行。',
      TH: 'ติ๊กแถวที่จะย้ายในคอลัมน์เลือก(☑)ด้านซ้าย'
    },
    '따를 저장된 정책이 없습니다.': {
      EN: 'No saved policy to follow.',
      JP: '従う保存済みポリシーがありません。',
      CH: '没有可跟随的已保存策略。',
      TH: 'ไม่มีนโยบายที่บันทึกให้ตาม'
    },
    '위 아래': {
      EN: 'Up / Down',
      JP: '上・下',
    ...
    '「활성(직접입력)」일 때만 직접 입력 가능합니다. 「기본」은 분할결제 안내 문구가 언어별로 표시됩니다. 로고설정과 무관하게 단독 표시할 수 있습니다.': {
      EN: 'Direct input only when Active (custom). Default shows split pay guide per language. Can be shown independently of logo setting.',
      JP: '「有効(直接入力)」時のみ入力可。「基本」は言語別の分割決済案内を表示。ロゴ設定とは無関係に単独表示できます。',
      CH: '仅「启用(自定义)」时可输入。「默认」按语言显示分次支付说明。可与徽标设置无关单独显示。',
      TH: 'กรอกได้เมื่อ「เปิดใช้(กำหนดเอง)」 「ค่าเริ่มต้น」แสดงคำแนะนำตามภาษา แสดงแยกจากโลโก้ได้'
    },
    ...
    '공개 결제 URL(/pay/업체코드 등) 입력 화면입니다. 간편(SIMPLE)은 성명·상품·금액·DirectCreditToken(카드 데이터는 토큰/CCD에 포함)만 받고, 전체(FULL)는 연락처·배송지까지 받습니다. <strong>브라우저 탭 이름</strong>은 이 결제 폼 전용이며 하단 <strong>저장</strong>으로 DB에 반영됩니다. <strong>탭 파비콘</strong>은 총본사 브랜드 「파비콘 이미지」와 자동 연동됩니다(없을 때만 아래 업로드 폴백). 인라인/리다이렉트는 위 「URL 결제형 기본 방식」과 제공 여부로 결정됩니다.': {
      EN: 'Public payment URL entry screen (/pay/{merchantCode}, etc.). SIMPLE collects name, product, amount, and DirectCreditToken (card data stays in token/CCD); FULL also collects contact and shipping address. <strong>Browser tab title</strong> is for this form and is saved with bottom <strong>Save</strong>. <strong>Tab favicon</strong> auto-links to the headquarters brand Favicon (upload below is fallback only). Inline vs redirect follows the "URL payment default flow" and availability above.',
      JP: '公開決済URL（/pay/加盟店コード 等）の入力画面です。簡易（SIMPLE）は氏名・商品・金額・DirectCreditTokenのみ、全体（FULL）は連絡先・配送先まで取得します。<strong>ブラウザタブ名</strong>はこのフォーム専用で下部<strong>保存</strong>でDB反映。<strong>タブファビコン</strong>は総本社ブランドのファビコンと自動連動（無い場合のみ下のアップロードがフォールバック）。インライン/リダイレクトは上の「URL決済型の既定方式」と提供可否で決まります。',
      CH: '公开支付 URL（/pay/商户代码 等）输入界面。简易（SIMPLE）仅收集姓名、商品、金额、DirectCreditToken；完整（FULL）还收集联系方式与配送地址。<strong>浏览器标签名</strong>专用于本表单，底部<strong>保存</strong>写入 DB。<strong>标签图标</strong>自动连动总公司品牌网站图标（仅无图标时用下方上传回退）。内联/重定向由上方「URL 支付型默认方式」及是否提供决定。',
      TH: 'หน้าป้อนข้อมูล URL ชำระเงินสาธารณะ SIMPLE รับชื่อ·สินค้า·ยอด·DirectCreditToken เท่านั้น FULL เพิ่มที่อยู่ติดต่อ·ที่อยู่จัดส่ง <strong>ชื่อแท็บ</strong>ใช้กับฟอร์มนี้และบันทึกด้วย「บันทึก」ล่างสุด <strong>ไอคอนแท็บ</strong>เชื่อมกับ Favicon แบรนด์สำนักงานใหญ่โดยอัตโนมัติ (อัปโหลดด้านล่างเป็นทางสำรอง) INLINE/REDIRECT ตามค่าเริ่มต้น URL ด้านบน'
    },
    '공개 결제 URL(/pay/업체코드 등) 입력 화면입니다. 간편(SIMPLE)은 성명·상품·금액·DirectCreditToken(카드 데이터는 토큰/CCD에 포함)만 받고, 전체(FULL)는 연락처·청구지까지 받습니다. <strong>브라우저 탭 이름</strong>은 이 결제 폼 전용이며 하단 <strong>저장</strong>으로 DB에 반영됩니다. <strong>탭 파비콘</strong>은 총본사 브랜드 「파비콘 이미지」와 자동 연동됩니다(없을 때만 아래 업로드 폴백). 인라인/리다이렉트는 위 「URL 결제형 기본 방식」과 제공 여부로 결정됩니다.': {
      EN: 'Public payment URL entry screen (/pay/{merchantCode}, etc.). SIMPLE collects name, product, amount, and DirectCreditToken (card data stays in token/CCD); FULL also collects contact and billing address. <strong>Browser tab title</strong> is for this form and is saved with bottom <strong>Save</strong>. <strong>Tab favicon</strong> auto-links to the headquarters brand Favicon (upload below is fallback only). Inline vs redirect follows the "URL payment default flow" and availability above.',
      JP: '公開決済URL（/pay/加盟店コード 等）の入力画面です。簡易（SIMPLE）は氏名・商品・金額・DirectCreditTokenのみ、全体（FULL）は連絡先・請求先まで取得します。<strong>ブラウザタブ名</strong>はこのフォーム専用で下部<strong>保存</strong>でDB反映。<strong>タブファビコン</strong>は総本社ブランドのファビコンと自動連動（無い場合のみ下のアップロードがフォールバック）。インライン/リダイレクトは上の「URL決済型の既定方式」と提供可否で決まります。',
      CH: '公开支付 URL（/pay/商户代码 等）输入界面。简易（SIMPLE）仅收集姓名、商品、金额、DirectCreditToken；完整（FULL）还收集联系方式与账单地址。<strong>浏览器标签名</strong>专用于本表单，底部<strong>保存</strong>写入 DB。<strong>标签图标</strong>自动连动总公司品牌网站图标（仅无图标时用下方上传回退）。内联/重定向由上方「URL 支付型默认方式」及是否提供决定。',
      TH: 'หน้าป้อนข้อมูล URL ชำระเงินสาธารณะ SIMPLE รับชื่อ·สินค้า·ยอด·DirectCreditToken เท่านั้น FULL เพิ่มที่อยู่เรียกเก็บ <strong>ชื่อแท็บ</strong>ใช้กับฟอร์มนี้และบันทึกด้วย「บันทึก」ล่างสุด <strong>ไอคอนแท็บ</strong>เชื่อมกับ Favicon แบรนด์สำนักงานใหญ่โดยอัตโนมัติ (อัปโหลดด้านล่างเป็นทางสำรอง) INLINE/REDIRECT ตามค่าเริ่มต้น URL ด้านบน'
    },
    'URL 결제 입력 폼': {
    ...
    '공개 결제·매뉴얼 창 탭 아이콘은 총본사 브랜드 「파비콘 이미지」와 자동 연동됩니다. 아래 업로드는 총본사 파비콘이 없을 때만 폴백으로 쓰입니다. PNG·JPG, 1MB 이하. 서버에서 32×32 PNG로 변환됩니다.': {
      EN: 'Public payment and manual windows use the headquarters brand <strong>Favicon</strong> automatically. The upload below is a fallback only when the HQ favicon is empty. PNG/JPG, up to 1 MB; server converts to 32×32 PNG.',
      JP: '公開決済・マニュアル画面のタブアイコンは総本社ブランドの<strong>ファビコン画像</strong>と自動連動します。下のアップロードは総本社ファビコンがない場合のみフォールバックです。PNG/JPG、1MB以下。サーバーで32×32 PNGに変換されます。',
      CH: '公开支付与手册窗口标签图标会自动连动总公司品牌<strong>网站图标</strong>。下方上传仅在总公司图标为空时作为回退。PNG/JPG，最大 1MB；服务器转为 32×32 PNG。',
      TH: 'หน้าต่างชำระเงินสาธารณะและคู่มือใช้ไอคอนแท็บจาก<strong>Favicon</strong> แบรนด์สำนักงานใหญ่โดยอัตโนมัติ การอัปโหลดด้านล่างเป็นทางเลือกสำรองเมื่อไม่มีไอคอน HQ PNG/JPG ≤1MB เซิร์ฟเวอร์แปลงเป็น PNG 32×32'
    },
    '결제통화로직설정': {
    ...
    '공개 결제 폼의 <strong>카드 *</strong> 제목·안내 문단과 <strong>결제 금액</strong> 입력란 아래 통화(×100/÷100) 안내는 <strong>PG(결제대행사)별</strong>로 함께 저장됩니다. 금액 안내는 「내용 1」 위에서 노출 여부·문구를 넣습니다(비우면 페이지 기본 다국어 문구). <strong>URL 결제 결과 문구</strong>는 <code>pay-result.html</code> 및 결제 페이지 인라인 완료 카드의 성공/실패 큰 제목·하단 안내를 바꿉니다(비우면 기본 문구). 취소 화면은 실패 문구와 동일 설정을 씁니다. (탭 제목은 위 「URL 결제 폼 설정」·탭 파비콘은 총본사 브랜드 「파비콘 이미지」와 자동 연동입니다.) URL 결제 연동(<strong>연동용도 URL결제</strong>) PG만 선택할 수 있습니다. 입력 후 <strong>저장</strong>으로 아래 목록에 넣고, 목록에서 <strong>활성</strong>을 켜야 반영됩니다. <strong>저장(다국어)</strong>은 본사 API가 MyMemory로 프록시하여 ENG·CHN·JPN·THA 초안을 채웁니다. 화면 맨 아래 <strong>저장</strong>으로 DB에 반영합니다. 총판 로고가 있으면 결제 폼 상단은 ICOPAY 대신 로고가 나옵니다(별도 연동).': {
      EN: 'Card section title/body and the amount-field footnotes (×100/÷100) are stored <strong>per PG</strong>. Amount note visibility/text is set above “Body 1” (empty = page default i18n). <strong>URL payment result copy</strong> changes success/fail titles on pay-result.html and inline completion cards. Cancel uses the fail copy. (Tab title: “URL payment form settings” above; tab favicon auto-links to headquarters brand Favicon.) Only URL-pay PGs may be selected. Use <strong>Save</strong> to add rows, then enable <strong>Active</strong>. <strong>Save (i18n)</strong> drafts ENG/CHN/JPN/THA via HQ API (MyMemory). Persist with bottom <strong>Save</strong>. Distributor logo replaces ICOPAY header when configured.',
  ...
  /* API 한글 라벨 — /hq/notifyInbound 목록·상세 (HqNotifyInboundQueryService) */
    ...
    '마스킹 카드번호(앞6+***+뒤4)·업체코드·업체명·고객성명·사유를 입력하고 [등록]을 누르세요. 등록된 카드는 전 가맹점 결제에서 차단되며, 업체코드·업체명은 출처 표시용입니다.': {
      EN: 'Enter masked card (first 6 + *** + last 4), merchant code, merchant name, customer name, and reason, then click Register. The card is blocked for all merchants; code and name are source info only.',
      JP: 'マスクカード番号(先頭6+***+末尾4)·加盟店コード·加盟店名·顧客氏名·理由を入力し[登録]を押してください。登録カードは全加盟店でブロックされ、コード・店名は出典表示用です。',
      CH: '输入掩码卡号(前6+***+后4)、商户代码、商户名称、客户姓名和原因后点击登记。该卡在全商户拦截；代码与名称为来源信息。',
      TH: 'กรอกเลขบัตรมาสก์ รหัสร้าน ชื่อร้าน ชื่อลูกค้า และเหตุผล แล้วกดลงทะเบียน บัตรถูกบล็อกทุกร้าน รหัส/ชื่อร้านเป็นข้อมูลแหล่งที่มาเท่านั้น'
    },
    ...
    '이름(구분용)': { EN: 'Customer name', JP: '顧客氏名', CH: '客户姓名', TH: 'ชื่อลูกค้า' },
    '이름(구분)': { EN: 'Customer name', JP: '顧客氏名', CH: '客户姓名', TH: 'ชื่อลูกค้า' },
    '고객성명': { EN: 'Customer name', JP: '顧客氏名', CH: '客户姓名', TH: 'ชื่อลูกค้า' },
    ...
    '기간정책': { EN: 'Period policy', JP: '期間ポリシー', CH: '周期政策', TH: 'นโยบายช่วงเวลา' },
    ...
    '지속적인 오류로 결제 서비스가 중지되었습니다.': {
      EN: 'The payment service has been suspended due to repeated errors.',
      JP: '継続的なエラーにより決済サービスが停止されました。',
      CH: '由于持续错误，支付服务已停止。',
      TH: 'บริการชำระเงินถูกระงับเนื่องจากข้อผิดพลาดต่อเนื่อง'
    },
    '서비스가 중지되었습니다.': {
      EN: 'The payment service has been suspended due to repeated errors.',
      JP: '継続的なエラーにより決済サービスが停止されました。',
    ...
    '이 가맹점은 결제 서비스가 중지로 설정되어 있습니다.': {
      EN: "This merchant's payment service is set to suspended.",
      JP: 'この加盟店の決済サービスは停止に設定されています。',
      CH: '该商户的支付服务已设置为停止。',
      TH: 'บริการชำระเงินของร้านค้านี้ถูกตั้งค่าเป็นระงับ'
    },
