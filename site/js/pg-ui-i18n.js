...
(function (g) {
  ...
  var STATIC = {
    ...
    '정산배포: PENDING 만 표시. 과거 DB가 V101 백필로 전부 DISTRIBUTED였다면 운영 DB에 db/V111_settlement_publish_pending_reopen.sql 적용 후 목록이 채워집니다. 처음 열 때 정산기간이 비어 있으면 최근 1년입니다. 행 클릭 시 정산일 당일 해당 가맹 전체 거래를 아래에 표시합니다. 체크 후 정산배포 → DISTRIBUTED, 선택 홀딩 → HOLD.': {
      EN: 'Settlement distribution: shows PENDING only. If an older DB was fully backfilled to DISTRIBUTED (V101), run db/V111_settlement_publish_pending_reopen.sql on the production DB to repopulate this list. When opened with an empty period, the default is the last year. Click a row to load that merchant’s payments for the settlement calendar day. After selecting rows: Distribute → DISTRIBUTED; Selected hold → HOLD.',
      JP: '精算配布: PENDINGのみ表示。過去DBがV101バックフィルで全件DISTRIBUTEDの場合は、本番DBにdb/V111_settlement_publish_pending_reopen.sqlを適用すると一覧が埋まります。精算期間が空のまま開いたときは直近1年です。行をクリックすると精算日当日の当該加盟店の全決済取引を下に表示します。チェック後: 精算配布→DISTRIBUTED、選択ホールド→HOLD。',
      CH: '结算下发：仅显示 PENDING。若历史库经 V101 回填全部为 DISTRIBUTED，请在生产库执行 db/V111_settlement_publish_pending_reopen.sql 后列表才会出现数据。首次打开若精算期间为空，默认为最近一年。单击行可在下方加载该商户精算日当天全部支付。勾选后：下发→DISTRIBUTED；选择暂缓→HOLD。',
      TH: 'แจกจ่ายผลชำระ: แสดงเฉพาะ PENDING หาก DB เก่าถูก backfill เป็น DISTRIBUTED ทั้งหมด (V101) ให้รัน db/V111_settlement_publish_pending_reopen.sql บน DB จริงแล้วรายการจะกลับมา เมื่อเปิดโดยช่วงว่าง ค่าเริ่มต้นคือ 1 ปีล่าสุด คลิกแถวเพื่อโหลดการชำระทั้งหมดของร้านในวันปฏิทินของวันชำระ หลังเลือก: แจกจ่าย→DISTRIBUTED; พักที่เลือก→HOLD'
    },
    '정산배포 · 당일 거래 내역': {
    ...
    '지급예정일(+7영업일)': {
      EN: 'Payout due (+7 business days)',
      JP: '支払予定日（+7営業日）',
      CH: '预计拨付日（+7 营业日）',
      TH: 'กำหนดจ่าย (+7 วันทำการ)'
    },
    '정산완료': {
      EN: 'Settlement done',
      JP: '精算完了',
