...
<%--
  ChillPay 인라인 결제 JSP 예제.
  1) IcopayMerchantApi.sample.java → com.icopay.merchant.IcopayMerchantApi 로 컴파일
  2) icopay-config.example.properties → WEB-INF/classes/icopay-config.properties
  3) 본 JSP 를 가맹 WAS 에 배포
--%>
<%
    String error = "";
    String embedHtml = "";
    ...
    try {
        ...
        apiBase = request.getServletContext().getInitParameter("icopay.apiBaseUrl");
    ...
    if (api != null && "POST".equalsIgnoreCase(request.getMethod())) {
        ...
        String currency = trimOrDefault(request.getParameter("currency"), "JPY");
        String productName = trimOrDefault(request.getParameter("productName"), "상품");
        if (orderNo.isEmpty() || amount.isEmpty()) {
            ...
            try {
                ...
                Map<String, Object> prep = api.prepareInlineCheckout(IcopayMerchantApi.VENDOR_CHILLPAY, orderNo, amount, currency, productName, pageLang);
                if (!Boolean.TRUE.equals(prep.get("success"))) {
                    ...
                    if (token == null || token.isBlank() || "null".equals(token)) {
                        ...
                        embedHtml = api.buildEmbedHtml(IcopayMerchantApi.VENDOR_CHILLPAY, token, null, pageLang);
                    }
                }
            } catch (Exception ex) {
                error = ex.getMessage();
...
<head>
  <meta charset="UTF-8">
  <title>ICOPAY ChillPay Checkout (JSP sample)</title>
  <style>body{font-family:system-ui,sans-serif;max-width:640px;margin:2rem auto;padding:0 1rem}.err{color:#b02a37}</style>
</head>
<body>
  <h1>ChillPay 결제 (JSP 예제)</h1>
  <% if (!error.isEmpty()) { %><p class="err"><%= html(error) %></p><% } %>

  <% if (embedHtml.isEmpty()) { %>
  <form method="post">
    <p><label>주문번호 <input name="orderNo" required maxlength="20" value="ORD<%= System.currentTimeMillis() %></label></p>
    <p><label>금액 <input name="amount" required value="10000"></label></p>
    <p><label>통화 <input name="currency" value="JPY"></label></p>
